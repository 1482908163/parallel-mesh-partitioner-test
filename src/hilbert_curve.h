/* 
定义了空间填充曲线（SFC）相关的类和函数。主要用于将3D空间的坐标映射到一个一维的索引
示例
    // 定义空间边界
    ...
    // 创建 Hilbert 曲线对象
    sfc::hilbert_curve_3d<double> hilbert(xmin, ymin, zmin, xmax, ymax, zmax);

    // 待映射的点
    double x = 0.25, y = 0.5, z = 0.75;

    // 获取一维 Hilbert 索引
    std::int64_t idx = hilbert.index(x, y, z);

    std::cout << "Hilbert index: " << idx << std::endl;

    // 用索引反查空间坐标
    auto [x_back, y_back, z_back] = hilbert.coords(idx);

    std::cout << "Recovered coords: "
              << x_back << ", " << y_back << ", " << z_back << std::endl;
*/
